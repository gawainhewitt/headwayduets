...
let numberOfButtons = 8;// automatically generate circular synth based on this
...
let sampler; // variable within which to create the synth
...
let instrumentOneImage, instrumentTwoImage;
let instrumentImage;
let instrumentImages = [];
let instrumentScales = [];
let instrumentScale;

function preload() {
  instrumentOneImage = loadImage('assets/frenchhornedit.png');
  instrumentTwoImage = loadImage('assets/cello.png');
}
...
function setup() {  // setup p5
  ...
  instrumentImage = instrumentOneImage;
  instrumentImages = [instrumentOneImage, instrumentTwoImage];
  ...
  radius = width/9;
  r = width/3;
  instrumentScales = [radius*2.4, radius*3.0];
  instrumentScale = instrumentScales[0];
  backGroundColours = ['gold','darkorange'];
  backGroundColour = backGroundColours[0];
...
function welcomeScreen() {
  ...
  text("Headway Duets. Touch screen or click mouse or use keys QWERTYU", width/10, height/10, (width/10) * 8, (height/10) * 8);
}
...
function drawSynth() { // instead of using the draw function at 60 frames a second we will call this function when something changes
  console.log(`instrumentScale ${instrumentScale}`);
  background(backGroundColour); // background is grey (remember 5 is maximum)
  imageMode(CENTER);
  image(instrumentImage, width/2, height/2, instrumentScale, instrumentScale);
  for (let i = 0; i < numberOfButtons; i++) {
...
function changeSound(sound) {
  let _sound = sound-1;
  backGroundColour = backGroundColours[_sound];
  instrumentImage = instrumentImages[_sound];
  instrumentScale = instrumentScales[_sound];
  console.log(`background colour =${backGroundColour}`)
  sampler.add("C3", instrumentSounds.get(_sound));
  drawSynth();
}

const instrumentSounds = new Tone.ToneAudioBuffers({
  urls: {
  1: `cello_lyrical_2.flac`,
  0: `horn-tone-c3.mp3`
},
baseUrl: "/assets/"
})
...
function playSynth(i) {
  if(synthState[i] === 0) { // if the synth is not playing that note at the moment
    sampler.triggerAttack(notes[i]); // play the note
...
function stopSynth(i) {
  if(synthState[i] === 1) { // if the synth is playing that note at the moment
    sampler.triggerRelease(notes[i]); // stop the note
 ...
 selectBoxes("soundmenu"); //make a pretty soundmenu
 ...
 function handleMenu(menu, index) { // function to handle the menu selections and change scales and keys
   if(menu === "keymenu"){
     ...
     console.log("the instrument is "+index);
     sound = index;
     changeSound(sound);
